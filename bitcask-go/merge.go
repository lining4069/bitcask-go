...
const (
	mergeDirName     = "-merge"
	mergeFinishedKey = "merge.finished" // 标志merge完成数据文件中记录的key
)
...
func (db *DB) Merge() error {
	...
	//Merge操作过程
	//1.持久化当前的活跃文件，打开新的活跃文件；构建所有旧的数据文件数组，释放数据库锁
...
// getMergePath 获取merge操作临时bitcask实例的路径，默认为当前实例同级目录下 + mergeDirName后缀
